 ...
 <%@page import="java.sql.*" %>
   ...
   String sid=request.getParameter("sid");
    ...
    try{
    	...
    	PreparedStatement ps=con.prepareStatement("update  student3 join markcard1  on student3.id=markcard1.sid set institude_name=?,registration_no=?,branch=?,ECcode=?,ECname=?,ECfull=?,ECSmark=?,EMcode=?,EMname=?,EMfull=?,EMSmark=?,EEcode=?,EEname=?,EEfull=?,EESmark=?,CAcode=?,CAname=?,CAfull=?,CASmark=?,sid=? where mid=?");
